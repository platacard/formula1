function CPlayer(iX, iY, oParentContainer) {
  ...
  this._init = function (iX, iY, oParentContainer) {
    ...
    _oPlayer = createSprite(
      s_aSpriteSheetCharacters[PLAYER_CAR_TYPE],
      "center",
      0,
      0,
      0,
  ...
  this._increase = function (start, increment, max) {
    // with looping
    ...
    while (result >= max) {
      ...
      if (_aCbCompleted[ON_PLAYER_PASSED_LAP]) {
        _aCbCompleted[ON_PLAYER_PASSED_LAP].call(
          _aCbOwner[ON_PLAYER_PASSED_LAP],
          _iCurLap,
        );
      }
      ...
      if (_bStartSprint) {
        ...
        if (_aCbCompleted[ON_PLAYER_COMPLETE_LAP]) {
          _aCbCompleted[ON_PLAYER_COMPLETE_LAP].call(
            _aCbOwner[ON_PLAYER_COMPLETE_LAP],
            _iCurLap,
          );
        }
      }
  ...
  this.getRunDistance = function () {
    return (
      _iCurPositionZ + (_iCurLap - 1) * TRACK_LENGTH + ENEMY_FLANKING_Z_OFFSET
    );
  };
  ...
  this.setRank = function (iNewRank) {
    if (_iRank !== iNewRank) {
      ...
      if (_aCbCompleted[ON_PLAYER_CHANGE_RANK]) {
        _aCbCompleted[ON_PLAYER_CHANGE_RANK].call(
          _aCbOwner[ON_PLAYER_CHANGE_RANK],
          _iRank,
        );
      }
    }
  ...
  this.getFrontPlayerSegment = function () {
    return s_oGame.findSegment(
      _iCurPositionZ + SEGMENT_LENGTH + PLAYER_Z_FROMCAMERA,
    );
  };
  ...
  this.damageAnim = function (iRelativeX, iRelativeZ) {
    ...
    var iX = linearFunction(
      iRelativeX,
      -PLAYER_RELATIVE_DAMAGE_X_DISTANCE,
      PLAYER_RELATIVE_DAMAGE_X_DISTANCE,
      -120,
      120,
    );
  ...
  this._updateXMovement = function (dx, iSpeedRatio) {
    ...
    iCurveCentrifugalForce =
      (dx *
        iSpeedRatio *
        iSpeedRatio *
        _oPlayerSegment.curve *
        CENTRIFUGAL_FORCE) /
      TERRAIN_ADHERENCE;
  ...
  this.lateralCollision = function (iRelativeXPos, oEnemy) {
    ...
    var oThis = this;
    createjs.Tween.get(oProp)
      .to({ value: 4 * iDir }, 200, createjs.Ease.cubicOut)
      .call(function () {
        _iPlayerState = STATE_RUNNER_RUN;
      })
      .on("change", function () {
  ...
  this.update = function (dt) {
    ...
    if (
      _iPlayerState !== STATE_RUNNER_HURTED &&
      _iPlayerState !== STATE_RUNNER_DEATH
    ) {
    ...
    _iAccelerationRate = linearFunction(
      _iCurSpeed,
      0,
      PLAYER_MAX_SPEED,
      PLAYER_ACCELERATION,
    ...
    if (_bKeyAccelerate) {
      ...
      if (_iCurSpeed > 0 && !soundPlaying(s_aSounds["engine_reverse"])) {
        ...
        var iStartAudio = linearFunction(
          iSpeedRatio,
          0,
          1,
          soundDuration(s_aSounds["engine_reverse"]),
          0,
        );
