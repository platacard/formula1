function CInterface() {
  ...
  this._init = function () {
    ...
    _oCountdownTextStroke = new createjs.Text(
      "",
      " 300px " + PRIMARY_FONT,
      "#000",
    );
    ...
    if (s_bMobile) {
      ...
      _oButUp = new CGfxButton(
        _pStartPosUp.x,
        _pStartPosUp.y,
        oSprite,
        oInterfaceContainer,
      );
      _oButUp.addEventListenerWithParams(
        ON_MOUSE_UP,
        s_oGame.onKeyUp,
        s_oGame,
        KEY_UP,
      ...
      _pStartPosDown = {
        x: CANVAS_WIDTH / 2,
        y: CANVAS_HEIGHT - oSprite.height + 100,
      }; // Центр экрана по X
      _oButDown = new CGfxButton(
        _pStartPosDown.x,
        _pStartPosDown.y,
        oSprite,
        oInterfaceContainer,
  ...
  this.refreshButtonPos = function () {
    ...
    if (s_bMobile) {
      ...
      _oButLeft.setPosition(
        _pStartPosLeft.x + s_iOffsetX,
        _pStartPosLeft.y - s_iOffsetY,
      );
      _oButRight.setPosition(
        _pStartPosRight.x - s_iOffsetX,
        _pStartPosRight.y - s_iOffsetY,
      );
    }

    _oTachometer.updateOffset(s_iOffsetX, s_iOffsetY);

    _oMiniMap.setPosition(
      _pStartPosMiniMap.x - s_iOffsetX,
      _pStartPosMiniMap.y - s_iOffsetY,
    );
  ...
  this.refreshRaceTime = function (iValue) {
    _oLapIndicator.refreshRaceTime(iValue);
  };
  ...
  this.countDownGo = function () {
    ...
    _oCountdownTextStroke.scaleX = _oCountdownTextStroke.scaleY =
      _oCountdownText.scaleY;

    _oCountdownText.text = TEXT_GO;
    _oCountdownTextStroke.text = TEXT_GO;

    createjs.Tween.get(_oCountdownText)
      .wait(500)
      .to({ alpha: 0 }, 1000, createjs.Ease.cubicIn);
    createjs.Tween.get(_oCountdownTextStroke)
      .wait(500)
      .to({ alpha: 0 }, 1000, createjs.Ease.cubicIn);
  };
