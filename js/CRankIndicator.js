function CRankIndicator(iX, iY, oParentContainer, iSize) {
  ...
  this.setColorBasedOnRank = function (iPos) {
    ...
    if (iPos <= MIN_RANK_FOR_WIN) {
      if (!TYPE_CUP_COLOR[iIndexColor]) {
        debugger;
      }
      szColor = TYPE_CUP_COLOR[iIndexColor]
        ? TYPE_CUP_COLOR[iIndexColor][0]
        : TYPE_CUP_COLOR[iIndexColor.length - 1][0];
      szOutlineColor = TYPE_CUP_COLOR[iIndexColor]
        ? TYPE_CUP_COLOR[iIndexColor][1]
        : TYPE_CUP_COLOR[iIndexColor.length - 1][1];
    }
