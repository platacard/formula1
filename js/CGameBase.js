...
*/
  ...
  this._oLevelBuilder = new CLevelBuilder(
    this._oPlayer,
    this._aCars,
    this._aEnemy,
    this._oElementContainer,
...
CGameBase.prototype.playerLapPassed = function () {
  this._iHorizonOffset +=
    this._aSegments[this._aSegments.length - 1].p1.world.x;
};
...
CGameBase.prototype._checkAmbientCollision = function (segment) {
  ...
  for (var n = 0; n < segment.elements.length; n++) {
    ...
    var bOverlap = Util.overlap(
      this._oPlayer.getPosition().x,
...
CGameBase.prototype._checkEnemiesCollision = function (segment) {
  ...
  for (var n = 0; n < segment.enemy.length; n++) {
    ...
    var bOverlapXCondition = Util.overlap(
      this._oPlayer.getPosition().x,
      this._oPlayer.getPlayerWidth(),
      oEnemy.getOffset(),
      iEnemyW,
    ...
    var bStateCondition =
      oEnemy.getState() !== STATE_RUNNER_DEATH &&
      oEnemy.getState() !== STATE_RUNNER_HURTED;

    if (bOverlapXCondition && bStateCondition) {
      var bSpeedToCheckDamage =
        this._oPlayer.getCurSpeed() - oEnemy.getSpeed() >
