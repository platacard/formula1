...
CGameRace.prototype._startGame = function () {
  ...
  this._oPlayer.addEventListener(
    ON_PLAYER_COMPLETE_LAP,
    this.playerLapCompleted,
    this,
  );
  this._oPlayer.addEventListener(
    ON_PLAYER_CHANGE_RANK,
    this.playerRankChanged,
    this,
  );

  var aStartingGrid = this.getStartingGrid(
    STARTING_GRID_X_OFFSET,
    STARTING_GRID_Z_OFFSET,
    TYPE_CAR_IN_GAME.length,
  );
...
CGameRace.prototype.createOpponent = function (
  iType,
  oDifficultyParams,
  iIndex,
) {
...
CGameRace.prototype.enemyLapCompleted = function (iCurLap, oOpponent) {
  ...
  if (iPercentDifference < 0) {
    ...
    iSkillBasedOnLapRemaining = linearFunction(
      iLapRemaining,
      1,
      this._iTotLap,
      iSkill,
...
CGameRace.prototype.getRankList = function () {
  ...
  aDistanceList.sort(function (a, b) {
...
CGameRace.prototype.trackCompleted = function () {
  ...
  if (iPlayerRank <= MIN_RANK_FOR_WIN) {
    ...
    var oNextLevelPanel = new CNextLevelPanel(
      iPlayerRank,
      this._iScore,
      this._iLevel,
    );
    ...
    $(s_oMain).trigger("save_score", this.raceTime);
  } else {
